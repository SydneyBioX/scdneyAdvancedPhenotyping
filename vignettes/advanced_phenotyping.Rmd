...
This workshop for the "Advanced Phenotyping in Single Cell Data Analysis" teaches workflows for the phenotyping of single cells. Phenotyping broadly refers to all procedures following pre-processing, normalisation and clustering of single cell data that attempts to characterize the identity and/or state of cells and the relationship between cells. 
...
* Familiarity with [previous workshop vignettes](add_links)

Relevant background reading for the workshop can be found below, including theoretical background we expect students to have.
...
| Characterizing a trajectory  | 40m  |
| Cell-cell communication      | 30m  |
| Concluding remarks           | 5m   |

### Workshop goals and objectives

The overall *Learning goals* of the workshop is to understand the basic workflows of two common approaches to phentoype the cell: trajectory analysis and cell-cell interaction. We will also discuss the general challenges associated with phenotyping single cells and future perspectives.
...
* create visualisation plots to visualise the trajectories and expression of genes
...
How do we identify the diverse phenotypes? Whilst single cell data enables us to interrogate the heterogeneity among the phenotypes and tease out different and distinct states, a cell's phenotype is the manifestation of a collective and integrated output of a set of molecules or networks that span across multiple modalities (transcriptome, genome, epigenome, and proteome etc). In this workshop, we begin to investigate this using single-cell transcriptomes. 

In this workshop, we will demonstrate two different approaches to phenotyping, trajectory analysis and cell-cell communication, using the mouse hippocampus dataset from La Manno et al. (2018).
...
We will use the dataset from [this paper](https://www.nature.com/articles/s41586-018-0414-6#additional-information). We provide the preprocessed version in this workshop. The original data can be access from GEO with the accession number GSE104323.
...
p4 <- plotDiffusionMap(sce, colour_by = "ClusterName") + ggtitle("27998 Cepo genes")
...
### 3.5 Extension: RNA velocity vs trajectory pseudotime inference
...
(Text copied from tradeSeq vignette)
...
# Only visualise the first 4 clusters
for (xx in 1:4) {
...
assoRes_lingeag1_top20 <- rownames(assoRes[order(assoRes$waldStat_1, decreasing = TRUE),])[1:10]
gene_to_test <- t(combn(assoRes_lingeag1_top20, 2))
